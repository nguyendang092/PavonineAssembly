...
export default function DetailedModal({ isOpen, onClose, area }) {
  ...
  // Hàm lấy ngày đầu tuần (thứ 2) từ ngày hiện tại
      ...
      if (snapshot.exists()) {
        ...
        for (const weekKey in weekData) {
          ...
          for (const model in models) {
            ...
            // Lặp qua tất cả ngày trong tuần hiện tại
            weekDates.forEach((date) => {
              if (modelData[date]) {
                const quantity = modelData[date];
  ...
  }, [selectedArea, currentWeekStart, forceFetch]);
      ...
      <div className="bg-white w-[90vw] h-[90vh] p-4 rounded shadow-lg flex flex-col">
        ...
        <div className="flex flex-col gap-2 mb-4 px-4 pt-2 pb-3 rounded-2xl shadow border border-[#e0e7ef] bg-white/70 backdrop-blur-sm w-full max-w-2xl mx-auto">
          <div className="flex flex-wrap gap-2">
            ...
            <div className="flex flex-col min-w-[120px] flex-1">
              <label className="text-xs text-gray-600 mb-1 font-semibold">
                Tuần hiện tại
              </label>
              <div className="border border-gray-300 rounded-lg px-2 py-1 bg-gray-100 text-gray-800 text-sm">
                    ...
                    onClick={() => {
                      ...
                      if (user && user.email) {
                        logUserAction(
                          ...
                          `Xem thêm chi tiết sản lượng khu vực: ${selectedArea}, tuần ${currentWeekNumber}`
                        );
                      }
                    }}
