...
// 2026년부터 QC와 지원부서를 제외합니다
const getTeamNamesForYear = (year) => {
  if (year >= 2026) {
      ...
      if (firebaseData) {
        ...
        if (firebaseData[selectedYear]) {
          ...
          if (yearData[0] && yearData[0].weeks) {
            // Cấu trúc mới, lọc dữ liệu dựa trên năm
            const filteredTeams = getTeamNamesForYear(selectedYear);
            const filteredData = yearData.filter((item) =>
              filteredTeams.includes(item.team)
            );
          ...
          // Nếu chưa có data cho năm này, dùng template với team lọc
          const filteredTeams = getTeamNamesForYear(selectedYear);
          const filteredTemplate = filteredTeams.map((name) => ({
            ...createTeamTemplate(),
            team: name,
          }));
        ...
        // Firebase chưa có data gì, dùng template với team lọc
        const filteredTeams = getTeamNamesForYear(selectedYear);
        const filteredTemplate = filteredTeams.map((name) => ({
          ...createTeamTemplate(),
          team: name,
