...
function AttendanceListDateOffToolbar({
  ...
  const offDayTriggerLabel = tl(
    "dayOffHolidayDropdownTrigger",
    "Ngày OFF / LỄ / NGHỈ BÙ",
  );

  const dayStatusBadge = isHolidayDay ? (
    <span
      className="absolute -right-1 -top-1 flex h-3.5 min-w-[1.1rem] items-center justify-center rounded-full border border-amber-200 bg-amber-500 px-0.5 text-[8px] font-black leading-none text-white shadow-sm"
      title={tl("dayKindHolidayBadge", "Lễ")}
    >
      L
    </span>
  ) : isCompensatoryDay ? (
    <span
      className="absolute -right-1 -top-1 flex h-3.5 min-w-[1.1rem] items-center justify-center rounded-full border border-teal-200 bg-teal-600 px-0.5 text-[8px] font-black leading-none text-white shadow-sm"
      title="Nghỉ bù"
    >
      B
    </span>
    ...
    <span
      className="absolute -right-1 -top-1 flex h-3.5 min-w-[1.1rem] items-center justify-center rounded-full border border-rose-200 bg-rose-600 px-0.5 text-[8px] font-black leading-none text-white shadow-sm"
      title="OFF"
    >
      O
    </span>
      ...
      {canManageOffDays ? (
        ...
        >
          <button
            ...
            aria-haspopup="menu"
            aria-label={offDayTriggerLabel}
            onClick={() => setOffHolidayDropdownOpen((open) => !open)}
            className={`relative inline-flex h-8 w-8 shrink-0 cursor-pointer items-center justify-center overflow-visible rounded-lg border-2 text-sm font-bold shadow-md outline-none transition-all ${
              offHolidayDropdownOpen
                ? "border-violet-500 bg-gradient-to-br from-violet-600 to-indigo-600 text-white shadow-lg shadow-violet-600/35"
                : "border-violet-400/90 bg-gradient-to-br from-white to-violet-50 text-violet-700 hover:border-violet-500 hover:shadow-lg hover:shadow-violet-500/20 dark:border-violet-500/70 dark:from-slate-900 dark:to-violet-950/80 dark:text-violet-200 dark:hover:border-violet-400 dark:hover:shadow-violet-900/40"
            }`}
            title={dayOffToolbarButtonTitle || offDayTriggerLabel}
          >
            <svg
                    ...
                    <div className="space-y-4 text-xs">
                      ...
                      <div className="rounded-xl border border-teal-200/90 bg-white p-3 shadow-sm dark:border-teal-900/60 dark:bg-slate-900">
                        <p className="flex items-center gap-2 border-l-4 border-teal-500 pl-2 text-[13px] font-extrabold uppercase tracking-wide text-teal-900 dark:border-teal-400 dark:text-teal-50">
                          ...
                          {tl("dayOffDropdownSectionCompensatory", "Nghỉ bù")}
                        </p>
