...
function S90dYieldRing({ yieldPct, label, sublabel, isValid = true, invalidHint = "" }) {
  if (!isValid || yieldPct == null) {
    return (
      <div className="s90d-kpi-card s90d-kpi-card--yield s90d-kpi-card--yield-invalid">
        ...
        {invalidHint ? (
          <p className="s90d-kpi-hint s90d-kpi-hint--warn">{invalidHint}</p>
        ) : null}
      </div>
    );
  }
...
export default function S90dKpiCards({ totalRow, processDetails = null }) {
  const rt = useReportT();
  const { fixedBoardSpecs, fixedBoardSpecsAllProcesses, processes } =
  ...
  const productYieldItems = useMemo(
    ...
    [processDetails, fixedBoardSpecs, fixedBoardSpecsAllProcesses, processes],
  );
  ...
  const invalidHint = rt(
    "kpiYieldChainIncomplete",
    "Thiếu SL ở công đoạn trước (vd. MC)",
  );
