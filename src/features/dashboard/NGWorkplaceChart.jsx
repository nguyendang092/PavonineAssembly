...
export default function NGWorkplaceChart() {
  ...
  return (
    ...
    >
      ...
      {/* Main content — loading chỉ phủ vùng nội dung, không chặn cả app */}
      <main
        className={`relative flex-1 flex flex-col lg:flex-row gap-4 lg:gap-8 px-4 sm:px-6 transition-all duration-300 ${
          sidebarOpen ? "ml-72" : "ml-0"
        }`}
        style={{ minHeight: 0, overflow: "hidden" }}
      >
        {dataLoading && (
          <div className="absolute inset-0 z-10 flex items-center justify-center rounded-xl bg-white/75 backdrop-blur-[1px] dark:bg-slate-950/75">
            <LoadingBlock />
          </div>
        )}
        {/* Chart 2/3 */}
